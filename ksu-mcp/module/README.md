...
1. 将 `ksu-mcp-server-v1.0.0.zip` 推送到手机（任意目录）。
...
> 升级自旧版本（v1.0.x）时，请先在 Manager 中**移除旧模块**再安装 v1.0.0；配置与数据目录 `/data/adb/ksu_mcp` 会自动保留。
...
| `tunnel` | 内网穿透配置：`enabled` 开启后开机自动拉起隧道客户端；`server` 为服务端 WebSocket 地址；`device` 为设备唯一标识（须与服务端登记一致）；`token` 为服务端下发的 tunnelToken；`ip`（可选）为**直连 IP**——手机 DNS 解析异常（代理/去广告模块改了 resolv.conf、或 Android 回退 `::1:53` 失败）时，填服务端公网 IP（如 `103.205.240.107`）可绕过域名解析直连，TLS 仍按域名校验证书，安全性不受影响 |
...
mcpd config-set --tunnel-ip 103.205.240.107                  # DNS 异常时直连兜底（可选）
```
