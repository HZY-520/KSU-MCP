...
3. **补齐心跳丢包指标**：任务 3.6 要求记录「延迟、丢包、重连次数」。此前只有延迟与重连，
   现新增 ping/pong 配对统计（`ping_sent` / `ping_lost` / `ping_loss_percent`），
   设备端 WebUI 与服务端管理台均以「心跳丢包 x% (n/m)」展示。
4. **Token 归位**：按任务 3.2 的面板划分，把 Token（显示/复制/重新生成）从「连接信息」
   移入「运行状态」面板；「连接信息」只保留三级地址与各自的鉴权提示。
5. **日志改增量渲染**：新增内容以旧内容为前缀时只对文本节点 `appendData` 增量（O(增量)），
   仅在日志轮转/截断或超限时整段重写，并保持「原本贴底才自动贴底」。

另外补充了 **CLI 契约测试**与**持续断网恢复测试**，并新增
[实现取舍说明](#十六实现取舍说明与任务要求的差异及理由) 记录三处有意识的实现差异。
...
| 心跳丢包统计 | 每次心跳配对 | 上一拍 ping 未在下一拍前收到 pong 即计一次丢包，输出 `ping_sent`/`ping_lost`/`ping_loss_percent`（服务端同样输出 `pingsSent`/`pingsMissed`/`lossPercent`） |
...
## 十六、实现取舍说明（与任务要求的差异及理由）

以下三处是**有意识的实现差异**，不是遗漏；每项都说明原因与替代方案。

### 16.1 网络状态监听：未使用 ConnectivityManager / 系统广播

任务 3.3 / 3.6 建议监听 Android `ConnectivityManager` 或网络变化广播。本模块是
**纯 Go、CGO_ENABLED=0 静态编译**的内嵌二进制（KernelSU 模块不能依赖 NDK/JNI 运行时），
无法直接调用 Java API。

**替代实现**：每 **5s** 读取内核态网卡地址表（`net.Interfaces`）+ 默认路由
（`/proc/net/route`），合成「网络环境指纹」，指纹变化即强制断开当前连接触发立即重连
（`src/tunnel.go` 的 `ifaceSig()`）。

**为什么等价**：对「网络切换后触发重连」这一个目标而言，`ConnectivityManager` 的
`onAvailable/onLost` 最终也正是反映为网卡与路由表变化。该实现无需 JNI，每周期开销为
两次系统调用 + 一次文件读取，可忽略。5s 周期满足「切换后 30s 内恢复」的验收要求
——隧道 e2e 实测：服务端不可用 75s（退避已增至上限）后恢复耗时 **15.6s**。

若后续引入 JNI 以获取更细事件（captive portal、网络质量变化），可在 `ifaceSig()` 旁
增加事件源，不影响现有逻辑。

### 16.2 日志列表：未做虚拟滚动，改为「有界 + 增量」

任务 3.1 建议「使用虚拟滚动或懒加载处理日志列表」。

**实现**：渲染上限 `MAX_LOG_LINES = 300`；新内容以旧内容为前缀时只对文本节点
`appendData` 增量（复杂度 O(增量行)）；仅在日志轮转/截断或超限时整段重写；
并保持「原本贴底才自动贴底」，不打断用户向上翻阅。

**理由**：`<pre>` 内是**单个文本节点**；虚拟滚动需要把日志拆成数百个独立 DOM 节点
并管理滚动位置与回收，在移动端 WebView 中的常量开销反而高于「单文本节点增量追加」。
渲染量已被行数上限严格界定，不存在无界增长。三条不变量在 `tests/webui_test.js` 中均有断言。

### 16.3 WebView 开关与静态资源缓存：不在模块可控范围

- 任务 3.1 提到「关闭不必要的 WebView 功能、启用硬件加速」。**WebView 配置由
  KernelSU Manager 的 `WebUIActivity` 决定**，模块内的 HTML 无法修改宿主进程的
  WebView 设置（是否允许 `file://` 跨源、是否启用硬件加速等）。
  模块侧能做的是「不依赖这些开关也能流畅」：不使用需要额外合成的
  `backdrop-filter`/固定背景、动画只用 `transform`/`opacity`、
  为吸顶栏建立独立合成层（`transform:translateZ(0)`）、用 `contain` 约束重排范围。
  若需真正调整 WebView 开关，需修改 KernelSU Manager 源码。
